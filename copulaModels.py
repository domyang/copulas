...
class cop_student(cop_mod):
    ...
    def __init__(self,uniforms,vv=None):
        ...
        def density(unifs):
            ...
            vecs=list(zip(*vecs))
            marginal_density=list(zip(*marginal_density))
            factor=spe.gamma((v+self.dim)/2)/(spe.gamma(v/2)*math.sqrt((math.pi*v)**self.dim*np.linalg.det(sigma)))
            cov_inv=sigma**(-1)
            ...
            for i in list(zip(*[vecs,marginal_density])):
                ...
                m=np.prod(i[1])
                if m>0:
                    temp=1/m*factor*math.exp(math.log(1+x*cov_inv*np.transpose(x)/v)*(-(v+self.dim)/2))
                else:
                    temp=0
                res.append(temp)
            return res
    ...
    def __init__(self,model,unifs,subsetFunctions=None,fact=5,visualize=False,redistribute=False,testMode=False):
        ...
        def distribute(count,visu=False):
            ...
            for subset in subsetFunctions:
                ...
                for ii in range(len(f_emp_tp)):
                    ...
                    for interval in find_under(f_tp[ii],f_emp_tp[ii],mi=0,ma=1):
                        ...
                        M=int(math.ceil(max(0,-find_min(f_tp[ii],f_emp_tp[ii],mi=interval[0],ma=interval[1])[0])*area*sset.quad_points[ii]))
                        if M==0:
                            continue
                    ...
                    if len(inters)>0:
                        def f_tp_modified(d, arg_ii=ii,arg_f_tp=f_tp,arg_inters=inters,
                                          arg_added_density=added_subset_density,arg_added_points=nb_points_added_in_quadrant,
            ...
            if visu&(dim==2):
                ...
                zz=[]
                for j in xxx:
                    zz.append(div_safe(compute_density(compute_inverse(j),f_emp),(compute_density(compute_inverse(j),f)*K)))
...
def norm_distribution(pdf,dim):
    ...
    def f(x):
        ...
        for pt in pdf_val:
            ...
            for d in range(dim):
                for e in range(dim):
                    jacobian[e*nb+current[e],d*nb+current[d]]=jacobian[e*nb+current[e],d*nb+current[d]]+pt
