...
class CopulaManager:
    ...
    def __init__(self,arg_date,vect,data,parameters,var_function=None):
        ...
        if len(date)==len(vect):
            ...
            while(cur<date[-1]):
                ...
                while cur>index:
                    print('Warning: possibly twice the same entry at date %s'% dt_object.fromtimestamp(index).__str__())
                    index=next(date_it)
                    next(vect_it)
                    for key in data.keys():
                        next(data_it[key])
            ...
            raise(RuntimeError('vect and date should be same length, (here %d,%d respectively)'%(self.length,len(date))))
    ...
    def update(self,parameters):
        ...
        if (parameters['type']=='Solar')&(parameters['kind']=='error'):
            if ('hour_sol' in data_keys)&(self.var_function is not None):
                ...
                for i in parameters['offsets']:
                    var_sol_temp.append(self.var_function(((i+time.hour+time.minute/60+time.second/3600)-rise)/(sete-rise)*12))
                for i in range(dim):
                    def f(x):
    ...
    def pprint(self):
        ...
        for offset in range(len(self.parameters['offsets'])):
            ...
            print('-> vectM (len=%d) [%0.2f, %0.2f, %0.2f, %0.2f, %0.2f, %0.2f...]\n'% (len(self.vectM[offset]), self.vectM[offset][0], self.vectM[offset][1], self.vectM[offset][2], self.vectM[offset][3], self.vectM[offset][4], self.vectM[offset][5] ))
            print('-> unif (len=%d) [%0.2f, %0.2f, %0.2f, %0.2f, %0.2f, %0.2f...]\n'% (len(self.unif[offset]), self.unif[offset][0], self.unif[offset][1], self.unif[offset][2], self.unif[offset][3], self.unif[offset][4], self.unif[offset][5] ))
                ...
                if len(l)==dim:
                    ...
                    if no_none:
                        ...
                        if param_actual['forecast_d']:
                            for i in range(dim):
                                b&=parameters['forecast_d'][i][0]<=data['forecast_d'][i]<=parameters['forecast_d'][i][1]
                    if not no_none:
                        return False
                else:
...
def check_param(parameters,data):
        ...
        param_temp={}
        param_actual={'window': False, 'date_range': False,'first_hour':False,'kind':False, 'forecast':False,'forecast_d':False}
        dim=0
