...
parameter={'offsets':[11, 14,16,18, 20],'date_range':('2010-07-01 18:00:00','2016-07-05 01:00:00'),'first_hour':(0,0)}
...
list_models.extend([lambda x, i=i: vines.D_vine(x,list_models=[[i],list1, list1, list1]) for i in list2])
list_models.extend([lambda x, i=i: vines.D_vine(x,list_models=[[i],[un], [un], [un]]) for i in list2])
list_models.append(lambda x: vines.D_vine(x, list_models=[list2, list1, list1, list1]))

res=ca.test_models(copula,list_models=list_models, end_incr=400)
results.compile_results(res, 'horse_races/TEST')
