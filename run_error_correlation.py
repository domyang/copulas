...
def test(options, action):
    ...
    if 'solar correlation' in action:
        ec.time_correlation('Solar', 'total', fig_nb=fn)
        fn = fn + 1
        ec.time_correlation('Solar', 'SP', fig_nb=fn)
        fn = fn + 1
        ec.time_correlation('Solar', 'NP', fig_nb=fn)
        fn = fn + 1
    ...
    if 'seasons comparison' in action:
        ...
        nb_period = 6
        date1 = dt.parse('07/01/14 00:00')
        date2 = dt.parse('06/30/15 23:00')
        ...
        for i in range(nb_period):
            ec.plot_copula_2d('Wind', 'NP', 'Wind', 'NP', options, date_range=[dates_list[i], dates_list[i + 1]],
                              hour_offset=offsets, fig_nb=fn)
            fn = fn + len(offsets)

    if 'time correlation' in action:
        ec.time_correlation('Wind', 'total', fig_nb=fn)
        fn = fn + 1
        ec.time_correlation('Wind', 'SP', fig_nb=fn)
        fn = fn + 1
        ec.time_correlation('Wind', 'NP', fig_nb=fn)
        fn = fn + 1
    ...
    if 'copula wind' in action:
        offsets = [11, 23, 24, 35]
        ec.plot_copula_2d('Wind', 'NP', 'Wind', 'NP', options, hour_offset=offsets, fig_nb=fn)
        fn = fn + len(offsets)
    ...
    if 'copula approximation' in action:
        ...
        ec.best_copula('Wind', 'NP', 'Wind', 'NP', options, nb_parts=6, nb_points_parts=2000, hour_offset=offsets,
                       fig_nb=fn, precision=20)
        fn = fn + 3 * len(offsets)

    if 'copula spline estimation' in action:
        offsets = [2]
        ec.copula_analysis('Wind', 'NP', 'Wind', 'NP', options, hour_offset=offsets, fig_nb=fn)
        fn = fn + 3 * len(offsets)
        ec.plot_copula_2d('Wind', 'NP', 'Wind', 'NP', options, hour_offset=offsets, fig_nb=fn, redistribute='Spline')
        fn = fn + len(offsets)
        ec.plot_copula_2d('Wind', 'NP', 'Wind', 'NP', options, hour_offset=offsets, fig_nb=fn, redistribute='Gaussian')
        fn = fn + len(offsets)

    if 'representative points' in action:
        offsets = [11, 17]
        list = ec.representative_points('Solar', 'NP', 'Solar', 'NP', options, nbPoints=8, distance_func='l2',
                                        hour_offset=offsets)
        fn += len(offsets)
    ...
    if 'parameter dependence' in action:
        ...
        res = []
        res.append(
            ec.plot_copula_2d('Wind', 'NP', 'Wind', 'NP', options, hour_offset=offsets, fig_nb=fn, kind='errors')[
                'characteristics'])
        fn += len(offsets)
...
def foo(options):
    ...
    offsets = [11, 23, 24, 35]
    ec.plot_copula_2d('Wind', 'NP', 'Wind', 'NP', options, hour_offset=offsets, fig_nb=fn, kind='forecasts')
    fn = fn + len(offsets)
    plt.show()
