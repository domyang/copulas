    ...
    for i in range(length):
        ...
        if index < i:
            ...
            if t['type']=='Solar':
                ...
                if t['kind'] == 'error':
                    temp['vect'] = [actual - forecast for (actual, forecast) in zip(res['act'], res['for'])]
...
def test_models(copula,win_days=90,list_models=None,repeat_win_days=False,win_forecast=0.2,visualize=False,start_incr=None,end_incr=None,
    ...
    res={'len':[],'log':[],'rank':[],'problem':[],'past_log':[],'selected_model':[],
         'proj_emd':[],'proj_quantile':[], 'wei_par':[]}

    res['parameters'] = parameters[0]
        ...
        def select_observations():
            for ser in range(nb_series):
                ...
                if not repeat_win_days:
                    parameters[ser]['date_range']=(str(dt.parse(obs[2])-relativedelta(days=win_days)),str(dt.parse(obs[2])-relativedelta(hours=1)))
                else:
            ...
            sorted_indexes=[sorted(range(copula.lengthM+1),key=forecast_tp[i].__getitem__) for i in range(nb_series)]
        ...
        ### computing the log likelihood ###
            try:
                if compare_dist:
                    ...
                    res_log.extend([den(obs[1])[0] for den in   ut.copula_to_densities(vects,cop_densities,log_return=True)])
                else:
                    res_log=[den(rank)[0] for den in ut.distribution_to_copula_densities(vects,densities,log_return=True)]
...
def copula_evolution(copula,win_days=45,day_interval=60,nb_max=5,method='color',uniforms=True,box=True):
    ...
    for i in range(nb_max):
        ...
        par[0]['date_range']=(start.__str__(),bound.__str__())
        ...
        if method=='manichean':
            ...
            date=[(j-date_start)/86400 for j in  copula.dateM]
...
def test_models_old(copula,win_days=45,repeat_win_days=False,win_forecast=0.2,visualize=False,start_incr=None,
    ...
    first=True
    last_time=time.time()
    res={'len':[],'log':[],'rank':[],'sum_pdf':[],'vec':[],'problem':[],'past_log':[],'selected_model':[],
    ...
    for obs in zip(*[forecasts[0],list(zip(*errors)),dates]):
        ...
        try:
            ...
            if compare_dist:
                ...
                res_log=[den(rank)[0] for den in ut.distribution_to_copula_densities(copula.vectM,densities,log_return=True)]
                res_log.extend([math.log(den([[r] for r in rank])[0]) for den in cop_densities])
