...
class CopulaManagerMS:
    ...
    def __init__(self,series,parameters_def,list_parameters=None,just_parameters=None):
        ...
        self.update(parameters_def,list_parameters=list_parameters,just_parameters=just_parameters,create=True,series=series)
    ...
    ### 1 - a few necessary checks and initialisations
        ...
        if create:
            ...
            for ser in series:
                if not {'date','vect','data','title'}.issubset(ser.keys()):
                    raise(RuntimeError('each element of series should have at least these entry key: {\'date\',\'vect\',\'data\',\'title\'}'))
        else:
        ...
        if list_parameters is not None:
            if (type(list_parameters)!=list)|(len(list_parameters)!=self.nbSeries):
                print('WARNING: list_parameters should be either None or a list same length as series; it won\'t be considered')
    ...
    ### 2 - creation of the single series copula-managers
        if create:
            ...
            for i in range(nbSeries):
                ...
                copulae.append(cop.CopulaManager(ser['date'],ser['vect'],ser['data'],par[i],var_function=ser['var_function']))
    ...
    ### 3 - taking only the dates that correspond
        ...
        current_indexes=[0 for i in range(nbSeries)]
        date_indexes=[[] for i in range(nbSeries)]
        dates_element=[0 for i in range(nbSeries)]
        dateM=[]
    ...
    ### 4 - retrieving the values (saved in vectM)
        ...
        for ser in series_to_consider:
            ...
            for key in copulae[ser].dataM.keys():
                dataM_tp[key]=[ list(map(copulae[ser].dataM[key][dim].__getitem__,date_indexes[ser])) for dim in range(copulae[ser].dim)]
    ...
    def plot(self,proj=[[]],which='unif',date=True):
        if proj==[[]]:
            ...
            if not argument_check:
                raise(RuntimeError('projection matrix must be either a numpy matrix or a list of list, with dimensions (2,%d)'%self.dim))
    ...
    for i in range(nb//2):
        for ii in [i,nb-1-i]:
            ...
            if interpolation=='linear':
                ...
                title='corner %d'%ii
                dic=ut.find_spline(x_rev,temp,visualize=visualize,title=title,increasingness_constraint=True,positiveness_constraint=True)
